# coding: utf-8

"""
    Core API

    Front is a customer operations platform that enables support, sales, and account management teams to deliver exceptional service at scale. Front streamlines customer communication by combining the efficiency of a help desk and the familiarity of email, with automated workflows and real-time collaboration behind the scenes.  With Front, teams can centralize messages across channels, route them to the right person, and unlock visibility and insights across all of their customer operations. More than 8000 businesses use Front to drive operational efficiency that prevents churn, improves retention, and propels customer growth.

    The version of the OpenAPI document: 1.0.0
    Created by: https://community.front.com
"""

from datetime import datetime, date
import typing
from enum import Enum
from typing_extensions import TypedDict, Literal, TYPE_CHECKING


class RequiredTeammateResponse(TypedDict):
    pass

class OptionalTeammateResponse(TypedDict, total=False):
    _links: typing.Dict[str, typing.Union[bool, date, datetime, dict, float, int, list, str, None]]

    # Unique identifier of the teammate
    id: str

    # Email address of the teammate
    email: str

    # Username of the teammate (used for \"@\" mentions)
    username: str

    # First name of the teammate
    first_name: str

    # Last name of the teammate
    last_name: str

    # Whether or not the teammate is an admin in your company
    is_admin: bool

    # Whether or not the teammate is available
    is_available: bool

    # Whether or not the teammate account has been blocked
    is_blocked: bool

    # Custom field attributes for this teammate
    custom_fields: typing.Dict[str, typing.Union[bool, date, datetime, dict, float, int, list, str, None]]

class TeammateResponse(RequiredTeammateResponse, OptionalTeammateResponse):
    pass
